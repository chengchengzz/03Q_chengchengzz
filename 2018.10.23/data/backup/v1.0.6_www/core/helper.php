...
if (!function_exists('code_validate')) {
   ...
   */
    function code_validate() {
        $token = base64_decode('I3NlcnZpY2VfZXlfdG9rZW4j');
        $token = trim($token, '#');
        $tokenStr = config($token);

        $keys = base64_decode('I3NlcnZpY2VfZXkj');
        $keys = trim($keys, '#');
        $md5Str = md5('~'.base64_decode(config($keys)).'~');

        if ($tokenStr != $md5Str) {
            $msg = base64_decode('I+aguOW/g+eoi+W6j+iiq+evoeaUue+8jOivt+WwveW/q+i/mOWOn++8jOaEn+iwouS6q+eUqOW8gOa6kOWFjei0uUV5b3VDbXPkvIHkuJrlu7rnq5nns7vnu58uIw==');
            $msg = trim($msg, '#');
            die($msg);
        }
