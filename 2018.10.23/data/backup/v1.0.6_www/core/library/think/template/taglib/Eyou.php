...
class Eyou extends Taglib
{
    ...
    protected $tags = [
        ...
        'arclist'    => ['attr' => 'channel,typeid,row,offset,titlelen,limit,orderby,noflag,flag,infolen,empty,mod,name,id,key'],
        'list'       => ['attr' => 'channel,typeid,pagesize,titlelen,orderby,noflag,flag,infolen,empty,mod,id,key'],
        'pagelist'   => ['attr' => 'listitem,listsize', 'close' => 0],
        'position'   => ['attr' => 'symbol,style', 'close' => 0],
        'type'       => ['attr' => 'typeid,type,empty,dirname,id,addtable'],
        'arcview'    => ['attr' => 'aid,empty,id,addtable'],
    ...
    public function tagList($tag, $content)
    {
        ...
        $parseStr .= ' $_result = $tagList->getList($param, '.$pagesize.', "'.$orderby.'");';
    ...
    public function tagType($tag, $content)
    {
        ...
        $empty  = htmlspecialchars($empty);
        $id     = isset($tag['id']) ? $tag['id'] : 'field';
        $addtable     = isset($tag['addtable']) ? $tag['addtable'] : '';
        $addtable  = $this->varOrvalue($addtable);
        ...
        $parseStr .= ' $__LIST__ = $tagType->getType($typeid, "'.$type.'", '.$addtable.');';
        $parseStr .= ' ?>';
    ...
    public function tagArcview($tag, $content)
    {
        ...
        $parseStr .= ' $__LIST__ = $tagArcview->getArcview($aid, '.$addtable.');';
        $parseStr .= ' ?>';
    ...
    public function tagFlink($tag, $content)
    {
        ...
        $empty  = htmlspecialchars($empty);
        $mod    = isset($tag['mod']) ? $tag['mod'] : '2';
        $row = !empty($tag['row']) && is_numeric($tag['row']) ? intval($tag['row']) : 30;
        $titlelen = !empty($tag['titlelen']) && is_numeric($tag['titlelen']) ? intval($tag['titlelen']) : 100;
