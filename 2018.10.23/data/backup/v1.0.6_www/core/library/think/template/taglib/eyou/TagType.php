...
class TagType extends Base
{
     ...
     */
    public function getType($typeid = '', $type = 'self', $addtable = '')
    {
        ...
        if (isset($result['nid']) && $result['nid'] == 'single' && preg_match("/(content)$/i", $addtable) == 1) {
            $row = M('single_content')->field('*')->where('typeid',$result['id'])->find();
            $row['content'] = isset($row['content']) ? eyou_htmlspecialchars_decode($row['content']) : '';
