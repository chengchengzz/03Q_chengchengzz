...
function func_common($fileElementId = 'uploadImage', $path = 'temp', $file_ext = "gif|jpg|png|jpeg"){
    ...
    if (empty($file)) {
        return ['errcode'=>1,'errmsg'=>'请选择上传的图片'];
    }
 ...
 * 多语言切换（默认中文）
 */
function func_lang($lang = '') 
{
    \think\Lang::setLangCookieVar('lang_var');
    ...
    $lang_var = cookie('lang_var');
    if (empty($lang_var) || $lang_var != $lang) {
        switch ($lang) {
            case 'en-us':
                cookie('lang_var', 'en-us');
            break;

            default:
                cookie('lang_var', 'zh-cn');
                break;
            //其它语言
        }
    }
