...
/*        $selected = 0;
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if ($aid) {
            	// ---------后置操作
            	model('Product')->afterSave($aid, $data);
            	// ---------end
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            if ($r) {
            	// ---------后置操作
            	model('Product')->afterSave($data['aid'], $data);
            	// ---------end
    ...
    {
        $path = I('filename','');
        M('product_img')->where("image_url = '$path'")->delete();
    }
    ...
    public function attribute_del()
    {
        ...
        if(!empty($id)){
            $r = M('ProductAttribute')->where("attr_id in ($id)")->delete();
            if($r){
                adminLog('删除产品参数-id：'.$id);
                respose(array('status'=>1, 'msg'=>'删除成功'));
            }else{
                respose(array('status'=>0, 'msg'=>'删除失败'));
            }
     ...
     */
    public function ajax_get_attr_input()
    {
    	$aid = I('aid/d');
    	$typeid = I('typeid/d');
        ...
        if (empty($str)) {
            $str = '<div style="font-size: 12px;text-align: center;">该主栏目还没有设置参数，若有需要请点击【<a href="'.url('Product/attribute_add', array('typeid'=>$typeid)).'">新增参数</a>】。</div>';
        }
        exit($str);
    }
