...
/*        $selected = 0;
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if ($aid) {
                // ---------后置操作
                model('Download')->afterSave($aid, $data);
                // ---------end
                adminLog('新增下载文档：'.$data['title']);
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            if ($r) {
                // ---------后置操作
                model('Download')->afterSave($data['aid'], $data);
                // ---------end
                adminLog('编辑下载文档：'.$data['title']);
                $this->success("操作成功!",U('Download/index', array('typeid'=>$post['typeid'])));
                exit;
            }
