    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if(M('channeltype')->where($map)->count('id') > 0){
                $this->error('该系统模型标识已存在，请检查', U('Channeltype/index'));
            }
        ...
        if(!empty($id)){
            foreach ($id_arr as $key => $val) {
                if (array_key_exists($val, $this->channeltype_system_id)) {
                    respose(array('status'=>0, 'msg'=>'系统预定义的模型不能删除'));
                }
            }

            $count = M('arctype')->where("channeltype in ($id)")->count('id');
            if ($count > 0){
                respose(array('status'=>0, 'msg'=>'该系统模型下有栏目，不允许删除，请先删除该系统模型下的栏目'));
            }  
            ...
            if ($r) {
                // \think\Cache::clear('channeltype');
                extra_cache('admin_channeltype_list_logic', NULL);
                adminLog('删除系统模型：'.implode(',', $title_list));
