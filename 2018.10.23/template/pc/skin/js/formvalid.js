...
var FormValid = function(frm) {
    ...
    this.required = function(inputObj) {
        ...
        if (typeof(inputObj) == "undefined" || inputObj.value.trim() == "" || inputObj.value.indexOf("输入")>0 || inputObj.value.indexOf("关键字")>0) {
	...
	this.filter = function (inputObj) {
 	...
 	this.isUsername = function (inputObj) {
        ...
        if (inputObj.value == '') {
            ...
            if (/^\d+$/.test(inputObj.value) || !/^[0-9a-zA-Z\@\.]{3,20}$/.test(inputObj.value)) {
				return false;
			}
			/*UisEmail = /([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)/;
			UisMobile = /^((\(\d{2,3}\))|(\d{3}\-))?(13|15|18)\d{9}$/;
            if (!UisEmail.test(inputObj.value) && !UisMobile.test(inputObj.value)) {
				return false;
			}*/
        }
        return true;
	}
	this.isUpwd = function (inputObj) {
        inputObj.value = inputObj.value.trim();
        if (inputObj.value == '') {
            return true;
        } else {
            if (/^\d+$/.test(inputObj.value) || !/^[0-9a-zA-Z]{6,20}$/.test(inputObj.value)) {
				return false;
			}
        }
        return true;
...
RegExps.istelsj = /(^((\(\d{2,3}\))|(\d{3}\-))?(13|15|18)\d{9}$)|(^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$)/;
...
RegExps.isCode = /^[A-Za-z0-9]{4}$/;
//RegExps.isUname =/^\s*[.A-Za-z0-9_-]{3,30}\s*$/;